						...
						<?php while($row_od = mysqli_fetch_assoc($query)){ ?>
							<tr>
								...
								<th class="btn-actions">
									<a href="SuaKH.php?id=<?php echo $row_od['id_kh'] ?>"><button class="btn btn-primary" type="submit" name="sbm" onclick="return confirm('Bạn có muốn sửa không?')">Sửa</button></a>
									<a href="XoaKH.php?id=<?php echo $row_od['id_kh'] ?>"><button class="btn btn-danger" type="submit" name="sbm" onclick="return confirm('Bạn có muốn xóa không?')">Xóa</button></a>
								</th>
							</tr>
						<?php } ?>
					</tbody>
